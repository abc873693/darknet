...
void test_list_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh,
    float hier_thresh, int dont_show, int ext_output, int save_labels, char *outfile, int letter_box)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    ...
    while (1) {
        ...
        draw_detections_v3(im, dets, nboxes, thresh, names, alphabet, l.classes, ext_output);
        char result[100];
        sprintf(result, "out/%s", basename(input));
        save_image(im, result);
        if (!dont_show) {
            show_image(im, result);
        }
...
void run_detector(int argc, char **argv)
{
    ...
    else if (0 == strcmp(argv[2], "test_data")) test_list_detector(datacfg, cfg, weights, filename, thresh, hier_thresh, dont_show, ext_output, save_labels, outfile, letter_box);
